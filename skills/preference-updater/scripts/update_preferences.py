...
def adjust_weight(preferences, category, delta):
    ...
    return {"old_weight": current_weight, "new_weight": new_weight, "delta": delta}
...
def add_research_area(preferences, category, weight, keywords_str):
    ...
    preferences["research_areas"][category] = {"weight": weight, "keywords": keywords}
...
def main():
    ...
    parser.add_argument(
        "--preferences", default="user_preferences.json", help="Preferences file path"
    )
    parser.add_argument(
        "--action",
        required=True,
    ...
    parser.add_argument(
        "--weight-delta", type=float, help="Weight adjustment (-1.0 to 1.0)"
    )
    parser.add_argument(
        "--weight", type=float, help="Initial weight for new area (0.5-1.0)"
    )
    ...
    if args.action == "add-keywords":
        ...
        if add_research_area(preferences, args.category, args.weight, args.keywords):
            action_summary = (
                f"Added research area {args.category} (weight: {args.weight})"
            )
            success = True
