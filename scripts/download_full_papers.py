...
Downloads papers from scored_papers_summary.json using a two-tier strategy:
...
- {arxiv_id}.html (if available)
- {arxiv_id}.pdf (fallback)
- {arxiv_id}.txt (extracted from PDF)
...
class PaperDownloader:
    ...
    def _extract_text_from_pdf(self, pdf_path: Path, txt_path: Path) -> Tuple[bool, int]:
        ...
        try:
            ...
            # Add this line to delete the PDF after successful extraction
            pdf_path.unlink()
            
            size_kb = len(text) / 1024
            print(f"    ✓ Text extracted ({size_kb:.1f} KB, {len(text.split())} words)")
            print(f"    ✓ PDF deleted (keeping .txt only)")
    ...
    def print_summary(self):
        ...
        print(f"HTML downloads:    {self.stats['html_success']} "
              f"({self.stats['html_success']/max(self.stats['total'],1)*100:.1f}%)")
        print(f"PDF fallbacks:     {self.stats['pdf_fallback']} "
...
def main():
    ...
    # Check if scored_papers_summary.json exists
