...
def generate_markdown(digest: dict) -> str:
    ...
    stats_section = f"**Papers in this digest:** {selected_count} selected from {total_reviewed} top candidates"
...
def main():
    ...
    parser.add_argument(
        "--input",
        required=True,
        help="Input digest JSON file (e.g., digest_2026-02-04.json)"
    )
    parser.add_argument(
        "--output",
        help="Output Markdown file (default: replaces .json with .md)"
    )
    ...
    if args.output:
        output_path = workspace / "resources/digests" / args.output
    else:
