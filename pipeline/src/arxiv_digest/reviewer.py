...
def _build_batch_analysis_prompt(
    ...
    return (
        ...
        "You are reviewing a paper for a colleague. Your colleague's time is limited; give them a review that is precise, honest, and grounded in technical detail.\n\n"
        "## Colleague's research interests:\n"
        f"{interests_text}\n\n"
        f"Analyze each of the following {len(batch)} papers:\n"
        f"{papers_text}\n\n"
        "## Review Task\n\n"
        "Think carefully about the paper before writing your review. Explain your justification before producing the following five fields *for each paper*:\n\n"
        "1. **arxiv_id**: the paper's arxiv_id (as given above)\n"
        "2. **summary**: — Three paragraphs, one each for:\n"
        "  a) The problem: what it is, why it matters, what gap this paper fills.\n"
        '  b) The approach: name the key mathematical objects, algorithms, or theoretical framework precisely. Avoid vague phrases like "using deep learning."\n'
        "  c) The results: what was demonstrated, proved, or measured. Quantify where the paper does.\n\n"
        "3. **relevance** — One paragraph. Name the specific research interest(s) this paper connects to, and explain the connection concretely. If the connection is weak or indirect, say so plainly rather than overstating relevance.\n\n"
        "4. **key_insight** — Two or three sentences. State the single most important idea the paper contributes. What would your colleague do differently — in their research or reading — after reading this paper?\n\n"
        "5. **score**: A float from 0 to 10 combining technical quality and relevance to stated interests. Use the full scale:\n"
        "  9–10: Landmark result directly on a stated interest; exceptional rigour.\n"
        "  7–8: Strong contribution with clear and direct relevance.\n"
        "  5–6: Solid work with partial or indirect relevance.\n"
        "  3–4: Good technical quality but weak connection to stated interests.\n"
        "  1–2: Marginal relevance at best; unlikely to inform the colleague's work.\n"
        "  0: Not relevant.\n"
        "  A paper that is technically excellent but off-topic should score no higher than 4.\n"
        "Return a JSON object with an 'analyses' array. Each element must have exactly these five fields: arxiv_id, summary, relevance, key_insight, score. Include one element per paper; do not omit any paper."
    )
