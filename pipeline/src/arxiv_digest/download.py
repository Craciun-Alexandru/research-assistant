...
class PaperDownloader:
    ...
    def __init__(self, output_dir: Path):
        ...
        self.stats = {"total": 0, "html_success": 0, "pdf_fallback": 0, "failed": 0, "skipped": 0}
    ...
    def _extract_text_from_html(self, html_path: Path, txt_path: Path) -> tuple[bool, int]:
        ...
        try:
            ...
            print("    ✓ HTML deleted (keeping .txt only)")
    ...
    def download_paper(self, paper: dict) -> dict:
        ...
        if html_success:
            ...
            if txt_success:
                ...
                return self._create_metadata(
                    paper, "html", html_size, txt_path, txt_extracted=True, txt_size=txt_size
                )
            else:
        ...
        print("    → HTML not available, falling back to PDF")
        ...
        return self._create_metadata(
            paper, "pdf", pdf_size, pdf_path, txt_extracted=txt_success, txt_size=txt_size
        )
    ...
    def _extract_text_from_pdf(self, pdf_path: Path, txt_path: Path) -> tuple[bool, int]:
        ...
        if PDF_LIBRARY is None:
            print("    ⚠️  No PDF library available, skipping text extraction")
            return False, 0
    ...
    def save_metadata(self):
        ...
        DOWNLOAD_METADATA_PATH.write_text(json.dumps(metadata_output, indent=2), encoding="utf-8")
    ...
    def print_summary(self):
        ...
        print(
            f"HTML→TXT:          {self.stats['html_success']} "
            f"({self.stats['html_success'] / max(self.stats['total'], 1) * 100:.1f}%)"
        )
        print(
            f"PDF→TXT:           {self.stats['pdf_fallback']} "
